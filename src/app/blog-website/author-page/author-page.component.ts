...
export class AuthorPageComponent implements OnInit {
  ...
  ngOnInit() {
    this.router.params.subscribe(result=> {
      //console.log(result)
      this.http.get<{status:string, msg:string, result:any}>('https://onewateracademy.org/api/blog/approvedauthor/'+result.id)
      .subscribe(result=>{
        //console.log(result);
  ...
  getblogs(id){
    this.http.get<{status:string, msg:string, result:any}>('https://onewateracademy.org/api/blog/authorapprovedblogs/'+id)
    .subscribe(result=>{
