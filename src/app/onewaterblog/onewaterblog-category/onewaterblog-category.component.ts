...
export class OnewaterblogCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe((res) => {
      this.catID = res.id;
      this.blogService.getBlogsForCategory(res.id).then((res: any) => {
        console.log(res);
  ...
  getCategories(){
    this.categories = [];
    this.blogService.getBlogCategories().then((res: any) => {
      res.forEach((element) => {
        if (element.name == "Innovation and Research")
          ...
          this.categories.push({
            name: "Legislative and Regulatory",
            id: element.id,
          ...
          this.categories.push({
            name: element.name,
            id: element.id,
            img: "assets/img/icons/default-category.svg",
          });
      });
